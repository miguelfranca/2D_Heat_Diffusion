...
CUDA_OBJ_FILES 	:= $(addprefix $(MAIN_OBJ_DIR)/,$(notdir $(CUDA_FILES:.cu=.o)))

LD_FLAGS := $(USER_FLAGS) $(VALGRIND) $(ROOT_LD) $(SFML_LD) $(CUDA_LD)
CC_FLAGS := $(USER_FLAGS) $(VALGRIND) $(ROOT_CD) $(SFML_CC) $(INCLUDES) -MMD

$(EXECUTABLE): $(MAIN_OBJ_FILES) $(LIBS_OBJ_FILES) $(CUDA_OBJ_FILES)
...
$(MAIN_OBJ_DIR)/%.o: %.cu $(MF) | $(MAIN_OBJ_DIR)
	@echo compiling $(GREEN) $(notdir $@) $(WHITE) depending $(YELLOW) $(notdir $<) $(WHITE)
	@$(CUDA_CC) -o $@ -c $<
